...
export GZOLTAR_CLI_JAR="/mnt/c/users/utente/desktop/uni/stage/gzoltar/com.gzoltar.cli/target/com.gzoltar.cli-$GZOLTAR_VERSION-jar-with-dependencies.jar"
...
export GZOLTAR_AGENT_RT_JAR="/mnt/c/users/utente/desktop/uni/stage/gzoltar/com.gzoltar.agent.rt/target/com.gzoltar.agent.rt-$GZOLTAR_VERSION-all.jar"
...
SRC_DIR="$SCRIPT_DIR/main/java"
TEST_DIR="$SCRIPT_DIR/test/java"
...
cd $SRC_DIR
javac -target 1.8 -source 1.8 -cp "$SCRIPT_DIR/main/java" "org/apache/commons/math/ode/nonstiff/EmbeddedRungeKuttaIntegrator.java" -d "$BUILD_DIR" || die "Failed to compile source code!"
cd ..
cd $TEST_DIR
javac -target 1.8 -source 1.8 -cp $JUNIT_JAR:$BUILD_DIR:$TEST_DIR:$SRC_DIR "org/apache/commons/math/ode/nonstiff/AdamsMoultonIntegratorTest.java" -d "$BUILD_DIR" || die "Failed to compile test cases!"
cd ..
...
java -cp $BUILD_DIR:$JUNIT_JAR:$HAMCREST_JAR:$GZOLTAR_CLI_JAR \
  com.gzoltar.cli.Main listTestMethods $BUILD_DIR \
    ...
    --includes "org.apache.commons.math.ode.nonstiff.AdamsMoultonIntegratorTest#polynomial" || die "Collection of unit test cases has failed!"
...
if [ "$INSTRUMENTATION" == "online" ]; then
  ...
  java -javaagent:$GZOLTAR_AGENT_RT_JAR=destfile=$SER_FILE,buildlocation=$BUILD_DIR,includes="org.apache.commons.math.ode.nonstiff.EmbeddedRungeKuttaIntegrator:org.apache.commons.math.ode.nonstiff.EmbeddedRungeKuttaIntegrator\$",excludes="",inclnolocationclasses=false,output="file" \
