...
cd $SRC_DIR
javac -target 1.8 -source 1.8 "org/apache/commons/lang3/StringUtils.java" -d "$BUILD_DIR" || die "Failed to compile source code!"
cd ..
cd $TEST_DIR
javac -target 1.8 -source 1.8 -cp $JUNIT_JAR:$BUILD_DIR:$TEST_DIR:$SRC_DIR "org/apache/commons/lang3/StringUtilsTest.java" -d "$BUILD_DIR" || die "Failed to compile test cases!"
cd ..
...
java -cp $BUILD_DIR:$JUNIT_JAR:$HAMCREST_JAR:$GZOLTAR_CLI_JAR \
  com.gzoltar.cli.Main listTestMethods $BUILD_DIR \
    ...
    --includes "org.apache.commons.lang3.StringUtilsTest#testReplace_StringStringArrayStringArray" || die "Collection of unit test cases has failed!"
...
if [ "$INSTRUMENTATION" == "online" ]; then
  ...
  java -javaagent:$GZOLTAR_AGENT_RT_JAR=destfile=$SER_FILE,buildlocation=$BUILD_DIR,includes="org.apache.commons.lang3.StringUtils:org.apache.commons.lang3.StringUtils\$",excludes="",inclnolocationclasses=false,output="file" \
